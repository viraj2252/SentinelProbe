...
class MockTargetRepository:
    """Mock implementation of TargetRepository for testing."""
    ...
    async def get_targets_by_job(self, job_id: int) -> List[Target]:
        ...
        """
        return [target for target in self.session.targets if target.job_id == job_id]
    ...
    async def delete_target(self, target_id: int) -> bool:
        ...
        """
        for i, target in enumerate(self.session.targets):
            if target.id == target_id:
                self.session.targets.pop(i)
                await self.session.commit()
                return True
        return False


class MockPortRepository:
    """Mock implementation of PortRepository for testing."""
    ...
    async def get_port(self, port_id: int) -> Optional[Port]:
        ...
        """
        for port in self.session.ports:
            if port.id == port_id:
                return port
        return None
    ...
    async def get_port_by_number(
        ...
        for port in self.session.ports:
            if (
                ...
                and port.port_number == port_number
    ...
    async def get_ports_by_target(self, target_id: int) -> List[Port]:
        ...
        """
        return [port for port in self.session.ports if port.target_id == target_id]
    ...
    async def delete_port(self, port_id: int) -> bool:
        ...
        Returns:
            bool: True if deleted, False if not found
        """
        for i, port in enumerate(self.session.ports):
            if port.id == port_id:
                self.session.ports.pop(i)
                await self.session.commit()
                return True
        return False
...
class MockServiceRepository:
    ...
    async def delete_service(self, service_id: int) -> bool:
        ...
        Returns:
            bool: True if deleted, False if not found
        """
        for i, service in enumerate(self.session.services):
            if service.id == service_id:
                self.session.services.pop(i)
                await self.session.commit()
                return True
        return False
