"""Test fixtures for SentinelProbe."""
...
from sentinelprobe.core.db import get_db_session, init_db
from sentinelprobe.core.mongodb import (
        ...
        if len(args) > 0 and hasattr(args[0], "whereclause"):
            ...
            if "FROM jobs" in query_str:
                ...
                job_objects = [
                    ...
                    if isinstance(obj, Job) and obj not in self.deleted_objects
                ]
                result.scalar = AsyncMock(
                    return_value=job_objects[0] if job_objects else None
                )
                ...
                task_objects = [
                    obj
                    for obj in self.objects
                    if isinstance(obj, Task) and obj not in self.deleted_objects
                ]
                result.scalar = AsyncMock(
                    return_value=task_objects[0] if task_objects else None
                )
...
async def mock_db_dependencies():
    ...
    with (
        patch("sentinelprobe.core.db.engine", mock_engine),
        ...
        patch(
            "sentinelprobe.core.mongodb.get_collection",
