...
def remove_unused_imports(file_path, imports_to_remove):
    ...
    for line_num, imports in imports_by_line.items():
        ...
        for import_name in imports:
            ...
            print(f"Trying to remove '{import_name}' from line {line_num}: {line.strip()}")
            ...
            if re.search(rf'^from\s+.+\s+import\s+.*{re.escape(import_name)}.*$', line):
                ...
                if f"{import_name}," in imports_str:
                    new_imports = re.sub(rf'{re.escape(import_name)},\s*', '', imports_str)
                # Handle imports without trailing comma but with preceding comma
                elif re.search(rf',\s*{re.escape(import_name)}(\s|$)', imports_str):
                    new_imports = re.sub(rf',\s*{re.escape(import_name)}(\s|$)', r'\1', imports_str)
                # Handle single import or last import in list
                else:
                    new_imports = re.sub(rf'\b{re.escape(import_name)}\b', '', imports_str)
...
def fix_specific_files():
    ...
    if os.path.exists(file_path):
        ...
        content = re.sub(r'from pydantic import .*Field.*', 
                         ...
                         content)
        content = re.sub(r'from sqlalchemy import .*Boolean.*', 
    ...
    if os.path.exists(file_path):
        ...
        content = re.sub(r'from typing import .*Union.*', 
                         ...
                         content)
        content = re.sub(r'from sentinelprobe.orchestration.models import .*JobType.*', 
